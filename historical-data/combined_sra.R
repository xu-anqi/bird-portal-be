...
wng <- read.csv('ranked_species_waarnemingen.csv')
wng <- wng[,-c(2,6,7,8)]

ebird <- read.csv('ranked_species_ebird.csv')
ebird <- ebird[,-c(2,6,7)]

inat <- read.csv('ranked_species_inaturalist.csv')
inat <- inat[,-c(2,6,7)]

d <- rbind(wng, ebird, inat)
dt <- aggregate(occurences ~ species, data=d, sum)
idx <- which(duplicated(d$species))
d <- d[-idx,]

d2 <- merge(dt, d, by='species', all.x=TRUE)
d2 <- d2[,-c(3)]
colnames(d2)[2] <- 'occurences'
d <- d2

anyNA(d$species)
anyDuplicated(d$species)
...
ggplot(data=d, aes(x=abundance_rank, y=occurences))+
  ...
  annotate(geom='text',col='red',x=350,y=40000,label='5.53 - 0.00917 x species rank')+
...
sum(p_vec)/log2(nrow(d)) #76% of the theoretical maximum entropy distribution (ie discrete uniform)

write.csv(d, file='ranked_species_combined.csv', row.names=FALSE)
